...
- **MSX native library bridge** — direct ctypes coupling with `libepanetmsx` (EPANET-MSX 2.0); loads `.msx` files unchanged; native libs bundled (v1.2.2)
...
# More control via the context manager
...
> The native library (`libepanetmsx`/`epanet2`) ships in `nzingaflow/lib/` — Linux x86-64 and
> Windows x86-64 are included. macOS is not yet built; build it yourself via EPANETMSX's `CMakeLists.txt`.
...
Direct coupling with the official EPANET-MSX C library via ctypes. Three layers on top of the `MSX_*` C API (EPANET-MSX 2.0).

> The native library (`libepanetmsx`/`epanet2`) ships in `nzingaflow/lib/` —
> Linux x86-64 and Windows x86-64 are included. For macOS: build it yourself via the `CMakeLists.txt`
> from [github.com/USEPA/EPANETMSX](https://github.com/USEPA/EPANETMSX) (supports Linux/macOS/Windows).
...
MsxSimulation(
    inp_path:  str,           # path to the EPANET .inp file
    msx_path:  str,           # path to the EPANET-MSX .msx reaction file
...
| `run(save_to_file=False, hyd_file=None)` | `MsxSimulationResult` | Run the full simulation; optionally via a previously saved hydraulics file |
...
lib.set_source(node, species, kind, level, pattern)
lib.close()
```

#### Which to use: `msx.py` or `msxlibrary.py`?
...
| **MSX native bridge** | Direct ctypes coupling with `libepanetmsx`; implemented in `msxlibrary.py` |
